    ...
    <Card mode={mode} getRootRef={getRootRef} vkuiClass={getClassName('ContentCard', platform)} style={style} className={className}>
      <Component {...tappableProps} vkuiClass="ContentCard__tappable">
        ...
        <div vkuiClass="ContentCard__body">
          {hasReactNode(subtitle) && <Caption Component="span" caps vkuiClass="ContentCard__caption" weight="semibold" level="3">{subtitle}</Caption>}
          {hasReactNode(header) && <Title vkuiClass="ContentCard__title" weight="semibold" level="3">{header}</Title>}
          {hasReactNode(text) && <Text Component="span" vkuiClass="ContentCard__text" weight="regular">{text}</Text>}
          {hasReactNode(caption) && <Caption Component="span" vkuiClass="ContentCard__caption" weight="regular" level="1">{caption}</Caption>}
        </div>
      </Component>
    </Card>
  );
